...
namespace Skyline.DataMiner.Utils.TableCleanup
{
    ...
    public class TableFilters
    {
        ...
        public class TableFiltersBuilder
        {
            ...
            public TableFilters Build(SLProtocol protocol, int cleanupMethodPid, int maxAlarmCountPid, int maxAlarmAgePid)
            {
                ...
                int deletionAmountMaxAlarmAge = Convert.ToInt32(maxAlarmAge / 100 * 20); // Remove 20% of the data
                switch (cleanupMethod)
                {
                    case CleanupMethod.RowAgeOrRowCount:
                        instance.Filters.Add(new MaximumRowCountFilter(maxAlarmCount, deletionAmountMaxAlarmCount));
                        instance.Filters.Add(new MaximumAgeFilter(maxAlarmAge, deletionAmountMaxAlarmAge));
                        break;
